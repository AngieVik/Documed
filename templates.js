...
const DOC_TEMPLATES = {
  ...
  informe_asistencia: {
    ...
    styles: {
      ...
      tableHeader:   { fontSize: 7.5, bold: true, color: "#ffffff", fillColor: "#334155", alignment: "center", margin: [2,3,2,3] },
    ...
    buildContent(data) {
      ...
      return [
        ...
        { margin:[0,0,0,8], stack:[{ text:"Lugar de Asistencia", style:"labelKey" },{ text:lugarAsistencia, style:"labelVal" }] },
        ...
        { margin:[0,0,0,8], stack:[{ text:"Anamnesis", style:"labelKey" },{ text:anamnesis, style:"labelVal" }] },

        // Constantes vitales
        constantesData.length > 0
          ? { margin:[0,0,0,8], table:{ widths:["auto","*","*","*","*","*"], headerRows:1, body:[
              [{ text:"", style:"tableHeader" },{ text:"TA (mmHg)", style:"tableHeader" },{ text:"FC (lpm)", style:"tableHeader" },{ text:"SpO2 (%)", style:"tableHeader" },{ text:"Temp (ºC)", style:"tableHeader" },{ text:"Glucemia", style:"tableHeader" }],
              ...constantesData,
            ]}, layout:{ hLineWidth:(i)=>(i===0||i===1?0.5:0.3), vLineWidth:()=>0.3, hLineColor:()=>"#cbd5e1", vLineColor:()=>"#cbd5e1", fillColor:(r)=>r%2===0?"#f8fafc":null } }
          : { text:"(Sin constantes vitales registradas)", style:"firmaPendiente", margin:[0,0,0,8] },

        { margin:[0,0,0,8], stack:[{ text:"Exploración Física", style:"labelKey" },{ text:exploracion, style:"labelVal" }] },

        // RESOLUCIÓN Y PLAN
        { text: "RESOLUCIÓN Y PLAN", style: "sectionHeader" },
        { margin:[0,4,0,4], stack:[{ text:"Juicio Clínico y Diagnóstico Presuntivo", style:"labelKey" },{ text:diagnostico, style:"labelVal", bold:true }] },
        { margin:[0,0,0,4], stack:[{ text:"Tratamiento Administrado In Situ", style:"labelKey" },{ text:tratamiento, style:"labelVal" }] },
        { margin:[0,0,0,8], columns:[
        ...
        { margin:[0,6,0,0], columns:[
          { width:"*", stack:[
            { text:"Firma del Paciente / Tutor Legal", style:"firmaLabel" },
            firmaPacienteContent,
            { canvas:[{ type:"line", x1:0, y1:0, x2:220, y2:0, lineWidth:0.5, lineColor:"#94a3b8" }], margin:[0,4,0,2] },
            { text:nombrePaciente, fontSize:7.5, color:"#475569" },
          ]},
          { width:20, text:"" },
          { width:"*", stack:[
            { text:`Facultativo: ${medico}${colegiado?" (Nº Col: "+colegiado+")":""}`, style:"firmaLabel" },
            firmaMedicoContent,
            { canvas:[{ type:"line", x1:0, y1:0, x2:220, y2:0, lineWidth:0.5, lineColor:"#94a3b8" }], margin:[0,4,0,2] },
        ...
        { fontSize:6.5, color:"#94a3b8", alignment:"justify", text: textoLegal },
      ];
    },
  },
